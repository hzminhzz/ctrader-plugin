...
public static class PositionManagementService
{
    public static ManagementResult Close(IEnumerable<Position> positions)
    {
        ...
        foreach (var position in positions.ToArray())
        {
            ...
            if (result.IsSuccessful) succeeded++;
            else lastError = result.Error.ToString();
        }
    ...
    public static ManagementResult PartialClose(IEnumerable<Position> positions, double closePercent, Func<string, Symbol> getSymbol)
    {
    ...
    public static TradeResult? CloseVolume(Position position, double closeVolumeInUnits, Symbol symbol)
    {
        ...
        var requestedRemaining = Math.Max(0, position.VolumeInUnits - closeVolumeInUnits);
        var normalizedRemaining = requestedRemaining <= 0 ? 0 : symbol.NormalizeVolumeInUnits(requestedRemaining, RoundingMode.Down);
        var decision = SmartVolumeReductionPlanner.Decide(position.VolumeInUnits, closeVolumeInUnits, symbol.VolumeInUnitsMin, normalizedRemaining);
        return decision.DecisionType switch
        {
            SmartVolumeReductionDecisionType.CloseAll => position.Close(),
            SmartVolumeReductionDecisionType.NoChange => null,
            _ => position.ModifyVolume(decision.NormalizedRemainingVolumeInUnits)
        };
    }

    public static ManagementResult MoveToBreakEven(IEnumerable<Position> positions, double offsetPips, Func<string, Symbol> getSymbol)
    {
        var attempted = 0;
        var succeeded = 0;
        string? lastError = null;
        foreach (var position in positions.ToArray())
        {
            attempted++;
            var symbol = getSymbol(position.SymbolName);
            var desiredStop = position.TradeType == TradeType.Buy ? position.EntryPrice + offsetPips * symbol.PipSize : position.EntryPrice - offsetPips * symbol.PipSize;
            var stopResult = ImproveStop(position, desiredStop, symbol);
            if (stopResult.Status is StopModificationStatus.Succeeded or StopModificationStatus.NoOp) succeeded++;
            else lastError = stopResult.Error;
        }
        return new ManagementResult(attempted, succeeded, lastError);
    }
    ...
    {
        if (!position.StopLoss.HasValue) return true;
